...
class DeviceConfigurationService
{
    ...
    public function handleConfigStatus(Camera $camera, array $payload)
    {
        return DB::transaction(function () use ($camera, $payload) {
            ...
            $appliedPayload = $payload["applied"] ?? true;
            $applied = is_array($appliedPayload)
                ? true
                : filter_var($appliedPayload, FILTER_VALIDATE_BOOLEAN);

            // Accept firmware status codes
            $isSuccess = in_array($status, [
                'SUCCESS',
                'CONFIG_SUCCESS',
            ], true);

            $newStatus = ($isSuccess && $applied)
                ? ConfigStatus::Applied->value
            ...
            if ($newStatus === ConfigStatus::Applied->value) {
                Camera::where('id', $camera->id)->update([
                    ...
                    'current_config_version' => DB::raw('desired_config_version'),
                    'current_config_hash' => DB::raw('desired_config_hash'),
                    'last_applied_at' => now(),
                ...
                $telemetry->fill([
                    'jpeg_quality' => $config['jpeg_quality'] ?? $telemetry->jpeg_quality,
                    'frame_size' => $frameMap[$config['frame_size'] ?? ""] ?? $telemetry->frame_size,
                    'mqtt_buffer' => $config['mqtt_buffer'] ?? $telemetry->mqtt_buffer,
                ]);

                $telemetry->config_version = $camera->desired_config_version;
                $telemetry->config_hash = $camera->desired_config_hash;
$telemetry->save();
            ...
            return true;
                ...
                Log::error('HANDLE_CONFIG_STATUS_EXCEPTION', [
                    'camera_id' => $camera->id,
                    'payload' => $payload,
 ...
 * Synchronizes desired/current state and auto-heal.
 */
public function handleTelemetryUpdate(Camera $camera, array $payload): void
{
    // Refresh latest camera data
    $camera->refresh();

    // Auto publish pending configuration when device reconnects
    $this->checkAndPublishPendingConfig($camera);

    // If telemetry already contains applied config version/hash,
    // synchronize current state.
    if (
        isset($payload['config_version']) &&
        isset($payload['config_hash'])
    ) {
        $version = (int) $payload['config_version'];
        $hash = (string) $payload['config_hash'];

        if (
            $camera->current_config_version !== $version ||
            $camera->current_config_hash !== $hash
        ) {
            $camera->update([
